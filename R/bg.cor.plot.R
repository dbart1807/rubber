bg.cor.plot <- function( bgs , metadata=NULL , cores=2 , ... ){
  ...
  lc=as.data.frame(matrix(unlist(mclapply(1:nrow(eg),function(x){
    val=cor(fl[[eg[x,1]]],fl[[eg[x,2]]],method="pearson")
    setTxtProgressBar(pb, x)
    return(val)
  },mc.cores=cores,mc.preschedule=T)),nrow=numfiles))
  cat("\n")
