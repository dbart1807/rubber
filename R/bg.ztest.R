bg.ztest <- function( testbg , controlbg , verbose=T , testname=NULL , controlname=NULL , windowsize=100 , pvalue=0.05 , padjmethod="fdr", threads=11 , mergewithin=50 ){
  ...
  possegm <- bed.merge(possegoutname,flank=mergewithin)
  negsegm <- bed.merge(negsegoutname,flank=mergewithin)
